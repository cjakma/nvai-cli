...
#   curl -fsSL https://raw.githubusercontent.com/<owner>/nvai-cli/main/scripts/install-user.sh | bash
#
...
if [ -n "${NVAI_SOURCE_DIR:-}" ]; then
  ...
  fail "NVAI_REPO_URL is required when installing from curl. Example: NVAI_REPO_URL=https://github.com/<owner>/nvai-cli.git bash install-user.sh"
fi
